...
function RenderPublishedBooks({ setEditingBook, setShowBookForm, handleAnalyticsClick, handlePromoteClick }) {
    ...
    const publishedBooksStats = {
        ...
        totalEarnings: publishedBooks.reduce((sum, book) => sum + book.earnings, 0),
        totalReaders: publishedBooks.reduce((sum, book) => sum + book.readers, 0),
        totalQuizParticipants: publishedBooks.reduce((sum, book) => sum + book.quizParticipants, 0)
    };
    ...
    const handleDeleteBook = (bookId) => {
        if (window.confirm('Are you sure you want to delete this book?')) {
            setPublishedBooks(publishedBooks.filter(book => book.id !== bookId));
        }
    };
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                        <div className="text-2xl font-bold text-gray-900">₹{publishedBooksStats.totalEarnings}</div>
